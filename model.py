...
def dilated_conv(inputs, num_filters):
    ...
    x = Conv2D(num_filters, 1, padding="same")(x) # "point-wise convolution"
    x = BatchNormalization()(x)
    x = Activation("relu")(x)
...
def build_model(input_shape):
    ...
    resnet50 = ResNet50(include_top=False, #'''need the FEATURE_EXTRACTOR part, not the IMAGE_CLASSIFICATION PART'''
                        weights="imagenet", 
                        input_tensor=inputs)
    ...
    """ Decoder """
    # < s4.shape, s3.shape, s2.shape, s1.shape >
